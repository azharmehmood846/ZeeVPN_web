...
export type ColorBendsProps = {
  ...
  /** Single bend color (preferred). */
  color?: string;
  /** Legacy multi-color palette (used when `color` is not set). */
  colors?: string[];
  rotation?: number;
  speed?: number;
  transparent?: boolean;
  ...
  intensity?: number;
  bandWidth?: number;
  /** Fade strength toward the top of the canvas (0–1). */
  fadeTop?: number;
};
...
void main() {
  ...
  if (uFadeTop > 0.001) {
    float topFade = smoothstep(0.72, 1.0, vUv.y);
    a *= 1.0 - uFadeTop * topFade;
  }
...
/** React Bits UI uses ~0.14; the shader expects ~6 for visible bands. */
function resolveShaderBandWidth(bandWidth: number) {
  if (bandWidth <= 1) {
    return (bandWidth / 0.14) * 6;
  }
  return bandWidth;
}
...
export function ColorBends({
  ...
  const shaderBandWidth = resolveShaderBandWidth(bandWidth);
  ...
  const applyUniforms = (material: THREE.ShaderMaterial) => {
    ...
    material.uniforms.uBandWidth.value = shaderBandWidth;
    material.uniforms.uFadeTop.value = fadeTop;

    const list = color ? [color] : colors ?? [];
    const arr = list.filter(Boolean).slice(0, MAX_COLORS).map(hexToVec3);
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uBandWidth: { value: shaderBandWidth },
    ...
    return () => {
      ...
      else if (useWindowResize) globalThis.removeEventListener("resize", handleResize);
      geometry.dispose();
      material.dispose();
      renderer.dispose();
